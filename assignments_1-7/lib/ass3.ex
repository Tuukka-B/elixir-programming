defmodule Ass3 do
  ...
  def part2 do
    ...
    combine_inputs = fn
      {input1, input2} when is_binary(input1) and is_binary(input2) ->
        IO.puts(
          "The combined string of \"#{input1}\" and \"#{input2}\" is \"#{input1 <> input2}\""
        )

      {input1, input2} when is_integer(input1) and is_integer(input2) ->
        IO.puts("The addition of numbers #{input1} and #{input2} is #{input1 + input2}")

      {input1, input2} ->
        IO.puts(
          "The input types of \"#{input1}\" and \"#{input2}\" are incompatible with each other"
        )
    end
  ...
  def find_lowest_denominator(number, divisor) do
    unless rem(number, divisor) == 0 do
      ...
      IO.puts("The number is divisible by #{divisor}")
    end
  end

  def ass3_mainloop do
    part1()
    part2()
  end
end
