defmodule Ass2 do
  ...
  def part1 do
    ...
    IO.puts(
      "Word count for a sentence \"#{base_string}\" is #{length(String.split(base_string))}."
    )
  ...
  def convert_to_pig_latin(word) do
    cond do
      ...
      String.match?(String.downcase(word), ~r/\W$/) ->
        Ass2.convert_to_pig_latin(String.slice(word, 0, String.length(word) - 1)) <>
      ...
      String.match?(String.downcase(word), ~r/^(squ)|(thr)|(str)/) ->
        result =
          String.slice(word, 3, String.length(word) - 1) <>
            String.slice(String.downcase(word), 0, 3) <> "ay"
      ...
      String.match?(
        ...
        ~r/^((ch)|(qu)|(squ)|(th)|(thr)|(sch)|(sm)|(str)|(st)|(gl)|(tr)|(fl)|(st))/
      ) ->
        result =
          String.slice(String.downcase(word), 2, String.length(word) - 1) <>
            String.slice(String.downcase(word), 0, 2) <> "ay"
      ...
      true ->
        cond do
          ...
          String.match?(String.downcase(word), ~r/^[bcdfghjklmnpqrstvwxz]/) ->
            result =
              String.slice(String.downcase(word), 1, String.length(word) - 1) <>
                String.slice(String.downcase(word), 0, 1) <> "ay"
