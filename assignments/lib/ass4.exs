...
"""
  ...
  def part2 do
    ...
    mainloop = fn(list_func, search_func, add_func, remove_func, main_func, books) ->
      input = String.trim_trailing(IO.gets("Choose an action from: list, search, add, remove, quit: "), "\n")
      case input do
        "list" ->
          list_func.(books)
    ...
    remove_func = fn(books) ->
      isbn_to_be_removed = String.trim_trailing(IO.gets("Enter ISBN of a book to be removed: "), "\n")
      IO.puts "All done!\n"
