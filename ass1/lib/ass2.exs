defmodule Ass2 do
  ...
  def part1 do
    ...
    baseString = "99 bottles of beer on the wall"
    IO.puts "Word count for a sentence \"#{baseString}\" is #{length(String.split(baseString))}."
  ...
  def part2 do
    # TO-DO: snake_case
    IO.puts("=== PART 2 ===")
    baseString = "Pattern Matching with Elixir. Remember that equals sign is a match operator, not an assignment"
    wordList = String.split(baseString)
    words = Enum.map(wordList, fn word -> Ass2.convert_to_pig_latin(word) end)

    IO.puts "\"#{baseString}\" converted to pig latin is \n\"#{Enum.join(words, " ")}\""
    IO.puts "== FINISHED =="
